...
def get_hand_digits_knn():
    ...
    if filecm.exists(path, file_train):
        ...
        for i in range(50):
            for j in range(100):
                cell_img = cells[i][j]
                cell_img2 = opencvcm.resize_by_max_contours(cell_img, 20, 20, 1, 1)
                # if j == 0:
...
def get_print_number_knn():
    ...
    if filecm.exists(path, file_train):
        # 从缓存文件中加载训练样本和结果
        logcm.print_info("load print number train data from cache file.")
        train = np.load(os.path.join(path, file_train))
        label = np.load(os.path.join(path, file_label))
    else:
        # 从图片中获取训练样本和结果
        logcm.print_info("load print number train data from Image.")
        # 获取numbers文件夹下所有文件路径
        ...
        for file_path in img_path:
            ...
            # 颜色空间转换（转换成灰度图）
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            opencvcm.save_tmp(gray, func_key, "gray", tmp_path, name)

            # 高斯模糊
            blur = cv2.GaussianBlur(gray, (5, 5), 0)
            opencvcm.save_tmp(blur, func_key, "GaussianBlur", tmp_path, name)

            # 自适应阈值可以看成一种局部性的阈值，通过规定一个区域大小，
            thresh = cv2.adaptiveThreshold(blur, 255, 1, 1, 11, 2)
            opencvcm.save_tmp(thresh, func_key, "adaptiveThreshold", tmp_path, name)
            ...
            for i in range(5):
                ...
                # 保存数字图片
                opencvcm.save_tmp(thresh1, func_key, "adaptiveThreshold", tmp_path, name + "-" + str(i + 1))
                opencvcm.save_tmp(thresh2, func_key, "adaptiveThreshold", tmp_path, name + "-" + str(j))
...
def find_hand_digit_knn(knn, roi, thresh_value, tmp_path=None, tmp_key=None):
    """
...
def find_print_number_knn(knn, roi, tmp_path=None, tmp_key=None):
    """
    使用KNN算法，判断感兴趣区域（ROI）中的数字。
    @:param knn KNN对象（已训练过）
    @:param roi 感兴趣区域
    ...
    # 函数名
    func_key = "find_print_number_knn"
    opencvcm.save_tmp(roi, func_key, "roi", tmp_path, tmp_key)

    # 统一大小
    resized_roi = cv2.resize(roi, (20, 40))
    opencvcm.save_tmp(resized_roi, func_key, "resize", tmp_path, tmp_key)

    thresh1 = cv2.adaptiveThreshold(resized_roi, 255, 1, 1, 11, 2)
    opencvcm.save_tmp(thresh1, func_key, "adaptiveThreshold", tmp_path, tmp_key)

    ## 归一化像素值
    normalized_roi = thresh1 / 255.

    # 展开成一行让knn识别
    sample1 = normalized_roi.reshape((1, 800)).astype(np.float32)
    sample1 = np.array(sample1, np.float32)

    # 矩阵展开成一维并转换为浮点数
    out = normalized_roi.reshape(-1, 800).astype(np.float32)
    # 通过KNN对象查找最符合的数字
    ret, result, neighbours, dist = knn.findNearest(out, k=1)
    number = int(result.ravel()[0])
    return number, normalized_roi
